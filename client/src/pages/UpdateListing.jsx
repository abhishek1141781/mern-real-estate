...
export default function UpdateListing() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      console.log("hello from useeffect: res", res);
  ...
  const hanldeImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      setImageUploadError("u can upload max 6 images");
      setUploading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.imageUrls.length < 1)
        return setError("you musht upolad at elslt 1 omage");
      if (formData.regularPrice < formData.discountPrice)
        return setError("Regular price must be bigger than discounted price");
      setLoading(true);
      setError(false);
      ...
      console.log("after sending to server: ", formData);
      const data = await res.json();
      setLoading(false);
        ...
        <div className="flex flex-col flex-1 gap-4">
          <p className="font-semibold">
            ...
            <span className="font-normal text-gray-700 ml-2">
              The first image will be conver(max 6)
            </span>
          </p>
